...
class EffectsBus:
    ...
    effect_type: EffectType
    params: EffectParams
    buffer_left: deque = field(default_factory=lambda: deque(maxlen=44100))
    buffer_right: deque = field(default_factory=lambda: deque(maxlen=44100))
    # Tracks whether the bus still has signal energy to drain
    has_tail: bool = False
    # How many consecutive silent buffers before we stop processing
...
class SynthEngine:
    ...
    # RMS level below which a bus is considered silent (used for tail detection)
    SILENCE_THRESHOLD = 1e-5
    # How many consecutive silent buffers before tail processing stops
    ...
    def _audio_callback(self, outdata, frames, time_info, status):
        ...
        # --- Pass 1: Render dry audio for all channels ---
        for ch_idx in range(8):
            ...
            # Feed sends regardless of note activity (silence keeps tails alive)
            self._feed_sends(ch_idx, dry, frames)

        # --- Pass 2: Drain all effect bus tails ---
        # This runs even when all channels are silent, allowing delay and
        # reverb tails to complete their natural decay
    ...
    def _render_channel(self, channel: int, frames: int) -> Optional[np.ndarray]:
        """Render dry audio for a channel, feeding effect send amounts."""
        if not self.active_notes[channel]:
            return None
        ...
        for note_data in self.active_notes[channel]:
            if note_data.get('is_drum'):
                ...
                chunk_len = end_idx - start_idx
                sig = drum_sample[start_idx:end_idx]
                sig = sig * voice.volume
                ...
                sig = self._generate_oscillator(
                    voice.voice_type, note_data, frames, voice.detune
                )
                envelope = self._generate_envelope(note_data, frames, voice.adsr)
                sig *= envelope
                sig, new_filter_state = self._apply_filter(
                    sig, voice.filter, note_data.get('filter_state', 0.0)
                )
                note_data['filter_state'] = new_filter_state
                sig *= note_data['velocity']
                sig *= voice.volume
        ...
        # Return the dry signal only.
        # _apply_effects_to_buses handles feeding sends and is called
        # separately from _audio_callback so tails drain even after notes end.
        return output

    def _feed_sends(self, channel: int, dry_signal: np.ndarray, frames: int):
        """
        Feed the dry channel signal into effect bus send queues.
        Called every buffer whether or not notes are active.
        When dry_signal is None (no active notes), feeds silence to allow
        tails to drain.
        """
        voice = self.channels[channel]
    
        if dry_signal is None:
            # Feed silence into buses that still have tails
        ...
        for effect_type, send_amount in send_map.items():
            ...
            if not bus.params.enabled:
                continue
            
            # Mark bus as having a tail whenever it receives non-silent input
            if send_amount > 0 and dry_signal is not None:
                rms = float(np.sqrt(np.mean(dry_l ** 2 + dry_r ** 2)))
    ...
    def _drain_effect_tails(self, frames: int) -> np.ndarray:
        """
        Process all effect buses, including tail drain when source is silent.
        Returns the summed stereo wet output of all buses across all channels.
        """
        output = np.zeros((frames, 2), dtype=np.float32)
        ...
        for effect_type, bus in self.effects_buses.items():
            ...
            # Gather the summed send input for this bus from all channels
            send_l = np.zeros(frames, dtype=np.float32)
            send_r = np.zeros(frames, dtype=np.float32)
            ...
            for ch_idx in range(8):
                ...
                # Use cached dry signal for this channel this buffer
                ch_dry = self._current_dry[ch_idx]
                if ch_dry is not None:
                    send_l += ch_dry[:, 0] * send_amount
                    send_r += ch_dry[:, 1] * send_amount
                # If ch_dry is None, we feed silence (already zero from np.zeros)

            # Process the bus with whatever input we have (may be silence)
            if effect_type == EffectType.CHORUS:
                wet_l, wet_r = self._process_chorus(send_l, send_r, bus)
            elif effect_type == EffectType.DELAY:
                wet_l, wet_r = self._process_delay(send_l, send_r, bus)
            elif effect_type == EffectType.REVERB:
                wet_l, wet_r = self._process_reverb(send_l, send_r, bus)
            elif effect_type == EffectType.COMPRESSION:
                wet_l, wet_r = self._process_compression(send_l, send_r, bus)
            elif effect_type == EffectType.CRUSH:
                wet_l, wet_r = self._process_crush(send_l, send_r, bus)
            else:
                continue

            # Check if this bus output has gone silent
            rms = float(np.sqrt(np.mean(wet_l ** 2 + wet_r ** 2)))
            if rms < self.SILENCE_THRESHOLD:
                bus.silent_buffer_count += 1
                if bus.silent_buffer_count >= self.TAIL_SILENCE_BUFFERS:
                    # Tail has fully decayed, stop processing this bus
                    bus.has_tail = False
                    bus.silent_buffer_count = 0
                    continue
            else:
    ...
    def set_effect_enabled(self, effect_type: EffectType, enabled: bool):
        """Enable or disable an effect, clearing state on disable."""
        bus = self.effects_buses[effect_type]
        bus.params.enabled = enabled
        
        if not enabled:
            # Clear persistent audio buffers
            bus.buffer_left.clear()
            bus.buffer_right.clear()
            
            # Clear effect-specific state
            state = self._effect_state[effect_type]
            if effect_type == EffectType.REVERB:
                # Force re-initialisation on next enable
                state.clear()
            elif effect_type == EffectType.COMPRESSION:
                state['env_left'] = 0.0
                state['env_right'] = 0.0
    ...
    def _process_compression(self, input_left: np.ndarray, input_right: np.ndarray,
                             bus: EffectsBus) -> tuple:
        """
        Feed-forward compressor for parallel compression bus.

        Returns the DIFFERENCE between compressed and dry signal.
        When added back to the dry signal in _apply_effects at send_amount=1.0,
        the result is fully compressed. At send_amount=0.5, it is half-compressed
        ...
        length = len(input_left)
        compressed_left  = np.zeros(length, dtype=np.float32)
        compressed_right = np.zeros(length, dtype=np.float32)
        ...
        for i in range(length):
            ...
            # Gain computation in dB domain
            env_db_l = 20.0 * np.log10(max(env_l, 1e-9))
            if env_db_l > threshold_db:
                over_l = env_db_l - threshold_db
            ...
            env_db_r = 20.0 * np.log10(max(env_r, 1e-9))
            if env_db_r > threshold_db:
                over_r = env_db_r - threshold_db
            ...
            compressed_left[i]  = in_l * gain_l
            compressed_right[i] = in_r * gain_r

        state['env_left']  = env_l
        state['env_right'] = env_r

        # Return the DIFFERENCE so that adding it to dry at send=1.0
        # produces fully compressed output
        diff_left  = compressed_left  - input_left
        diff_right = compressed_right - input_right
