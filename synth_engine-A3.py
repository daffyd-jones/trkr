...
class SynthEngine:
    ...
    def _process_delay(self, input_left: np.ndarray, input_right: np.ndarray,
        ...
        fb = min(abs(params.feedback), 0.95)
    
        # Calculate how many repeats until the echo decays below -60dB
        # f^n = 0.001  →  n = log(0.001) / log(f)
        # Then total history needed = n * delay_samples
        if fb > 0.01:
            repeats_needed = int(np.ceil(np.log(0.001) / np.log(fb)))
        else:
            repeats_needed = 1
        # Minimum 2 repeats, cap at something reasonable
        repeats_needed = max(2, min(repeats_needed, 50))
    
        required_size = delay_samples * repeats_needed + length
    
        # Resize buffers if needed
        if bus.buffer_left.maxlen < required_size:
            old_l = list(bus.buffer_left)
            old_r = list(bus.buffer_right)
            bus.buffer_left = deque(old_l, maxlen=required_size)
            bus.buffer_right = deque(old_r, maxlen=required_size)
    
        # Pre-fill with silence if history is too short for the first read
        while len(bus.buffer_left) < delay_samples:
            bus.buffer_left.appendleft(0.0)
            bus.buffer_right.appendleft(0.0)
    
        # Convert deque to list for indexed access
        hist_l = list(bus.buffer_left)
        hist_r = list(bus.buffer_right)
    
        output_left = np.zeros(length, dtype=np.float32)
        output_right = np.zeros(length, dtype=np.float32)
    
        # Working buffer: history + new samples appended one at a time
        work_l = hist_l[:]
        work_r = hist_r[:]
    
        for i in range(length):
            # Read from delay line: delay_samples behind the current write position
            read_pos = len(work_l) - delay_samples
    
            if read_pos >= 0:
                echo_l = work_l[read_pos]
                echo_r = work_r[read_pos]
            else:
                echo_l = 0.0
                echo_r = 0.0
    
            # Write: input + attenuated feedback from the delay output
            new_l = float(input_left[i]) + echo_l * fb
            new_r = float(input_right[i]) + echo_r * fb
            ...
            # Output is the echo only (wet signal)
            output_left[i] = echo_l
            output_right[i] = echo_r
    
        # Persist the tail: keep enough history for the full decay
        # The deque maxlen handles the trimming automatically from the left
        bus.buffer_left.clear()
        bus.buffer_right.clear()
    
        # Only need to keep the last required_size samples
        # work_l has grown by `length` samples, take the tail
        tail_start = max(0, len(work_l) - required_size)
        for s in work_l[tail_start:]:
            bus.buffer_left.append(s)
        for s in work_r[tail_start:]:
            bus.buffer_right.append(s)
